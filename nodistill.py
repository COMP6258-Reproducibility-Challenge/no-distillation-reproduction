...
def main(args):
    ...
    def assign_softlabels():
	    ...
	    # save the original labels
            torch.save(label_syn.cpu(), '../original_soft_labels.pt')
            noise_std = 0.05# or make this an arg like args.noise_std
            noise = torch.normal(mean=0.0, std=noise_std, size=label_syn.shape, device=label_syn.device)
            label_syn += noise
            label_syn = torch.clamp(label_syn, min=1e-6)  # avoid negatives
            label_syn /= label_syn.sum(dim=1, keepdim=True)  # re-normalize to keep valid distributions

            # Save new labels
            save_path = f'../noisy_labels_std{noise_std}.pt'
