...
def main(args):
    ...
    def assign_softlabels():
        ...
        if args.expt_type == 'ablation_k': # label swap experiment
            ...
            print("Running ablation_k experiment: preserving top-1 class, shuffling the rest.")

            # Save the original labels
            label_syn_before = label_syn.clone()

            for i in range(label_syn.size(0)):
                probs = label_syn[i]
                top_val, top_idx = torch.topk(probs, 1)  # Get top-1 class and value
                rest_indices = [j for j in range(len(probs)) if j != top_idx.item()]
                rest_vals = probs[rest_indices]

                # Shuffle the rest
                shuffled_vals = rest_vals[torch.randperm(len(rest_vals))]

                # Assign new probabilities
                new_probs = torch.zeros_like(probs)
                new_probs[top_idx] = top_val
                new_probs[rest_indices] = shuffled_vals
                label_syn[i] = new_probs

            # Save modified labels
            label_syn_after = label_syn.clone()
            import os
            ...
            save_path_before = get_unique_path(f'../../labels_org_IPC{args.ipc}_ExpEpoch{args.max_expert_epoch}.pt')
            save_path_after = get_unique_path(f'../../labels_swapped_IPC{args.ipc}_ExpEpoch{args.max_expert_epoch}.pt')

            torch.save(label_syn_before.cpu(), save_path_before)
            torch.save(label_syn_after.cpu(), save_path_after)


        if args.expt_type == 'add_noise':
            true_class_probability = args.true_class_probability if hasattr(args, 'true_class_probability') else 0.9
            
            # Build true_classes assuming synthetic data is in class order
            true_classes = []
            for c in range(num_classes):
                true_classes += [c] * args.ipc
            true_classes = torch.tensor(true_classes, device=label_syn.device)

            # Save original labels before adding noise
            save_path_before = f'../../labels_before_noise_IPC{args.ipc}_ExpEpoch{args.max_expert_epoch}.pt'
            
            torch.save(label_syn.clone().cpu(), save_path_before)
            print(f"[INFO] Original labels saved to {save_path_before}")

            import random

            for i in range(label_syn.shape[0]):
                true_class = true_classes[i]  # use actual class label, not argmax

                # Pick 10 random *other* classes (excluding true_class)
                other_classes = [j for j in range(num_classes) if j != true_class]
                selected = torch.tensor(random.sample(other_classes, 99), device=label_syn.device)

                # Generate random values for selected classes
                noise = torch.rand(99, device=label_syn.device)
                noise = noise / noise.sum() * (1.0 - true_class_probability)

                # Reset label vector and assign noise + true class prob
                label_syn[i] = 0
                label_syn[i][selected] = noise
                label_syn[i][true_class] = true_class_probability


            # Save modified labels
            save_path = f'../../noisy_labels_top{true_class_probability}_IPC{args.ipc}_ExpEpoch{args.max_expert_epoch}.pt'
            torch.save(label_syn.cpu(), save_path)
            print(f"[INFO] Noisy labels saved to {save_path}")
    ...
    if args.expt_type == 'tune_start': 
        ...
        ''' ablation 1: importantce of k-th element in softmax'''
            ...
            args.ablation_k = 1  # or any value
            expt_dict = {0: args.ablation_k}
            print(f"Running ablation_k experiment with k = {args.ablation_k}")
            
            

    elif args.expt_type == 'add_noise':
        expt_dict = {"noise_std": [0.05]}  # 🔧 Adjust the std as needed
        ...
        pass  # or call your label perturbation logic here
...
if __name__ == '__main__':
    ...
    parser.add_argument('--true_class_probability', type=float, default=0.9, help='Probability assigned to the true class when generating noisy soft labels.')
