...
def main(args):
    ...
    def assign_softlabels():
        ...
        if args.expt_type == 'low_entropy_boost':
            ...
            # Compute entropy for each label vector (assumes label_syn is [N, C])
            label_entropies = -torch.sum(label_syn * torch.log(label_syn + 1e-10), dim=1)  # shape [N]
          
            # Sort by entropy (ascending: lowest entropy first)
            sorted_indices = torch.argsort(label_entropies)
            
            # Select bottom-k samples
            bottom_k = args.num_low_entropy_images  # e.g., 0.5 * len(label_syn)
            bottom_k = int(bottom_k * label_syn.shape[0])
            bottom_k_indices = sorted_indices[:bottom_k]
            
            # Create uniform noise (same shape as label vectors)
            noise = torch.rand_like(label_syn[bottom_k_indices])
            noise = noise / noise.sum(dim=1, keepdim=True)  # Normalize to make it a valid distribution

            # Blend original low-entropy labels with noise
            print('Average entropy of bottom-k (pre-injection)= ', torch.mean(label_entropies[bottom_k_indices]))
            print('Average entropy of all (pre-injection)= ', torch.mean(label_entropies))
            alpha = args.entropy_boost_alpha  # e.g., 0.1
            label_syn[bottom_k_indices] = (
                (1 - alpha) * label_syn[bottom_k_indices] + alpha * noise
            )
            label_entropies_treated = -torch.sum(label_syn * torch.log(label_syn + 1e-10), dim=1)
            # print('Sums to 1 after?:', torch.sum(label_syn, dim=1))
            print('Average entropy of bottom-k (post-injection)= ', torch.mean(label_entropies_treated[bottom_k_indices]))
            print('Average entropy of all (post-injection)= ', torch.mean(label_entropies_treated))
            print(f"Injected noise into {bottom_k} lowest-entropy labels with alpha={alpha}")
...
if __name__ == '__main__':
    ...
    # Low entropy boost experiment
    parser.add_argument('--num_low_entropy_images', type=float, default=500, help='Number of low-entropy labels to modify')
    parser.add_argument('--entropy_boost_alpha', type=float, default=0.1, help='Blending factor for injected noise')
